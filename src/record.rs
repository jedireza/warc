...
mod streaming_trait {
    ...
    pub struct StreamingBody<T: Read + Seek>(T, u64);
...
impl<T: BodyKind> Record<T> {
    ...
    /// Create a new empty record with a streaming body.
    ///
    /// Using a `RecordBuilder` is more efficient when creating records from known data.
    ///
    /// The record returned contains the passed body stream, and the following fields:
    /// * WARC-Record-ID: generated by `generate_record_id()`
    /// * WARC-Date: the current moment in time
    /// * WARC-Type: resource
    /// * WARC-Content-Length: `body.bytes().count()`
    pub fn with_stream<R: Read + Seek>(mut stream: R) -> std::io::Result<Record<StreamingBody<R>>> {
        let len = {
